...
package com.violet.hikikomori.view.media.camera.record
...
import com.violet.hikikomori.BuildConfig
import com.violet.hikikomori.R
import com.violet.hikikomori.databinding.FragmentPreviewBinding
...
class PreviewFragment : BaseBindingFragment<FragmentPreviewBinding>() {
    ...
    private val pipeline: Pipeline by lazy {
        if (args.useHardware) {
            HardwarePipeline(
                ...
                characteristics, encoder, mBinding.viewFinder
            )
        } else {
            SoftwarePipeline(
                args.width, args.height, args.fps, args.filterOn,
    ...
    private val animationTask: Runnable by lazy {
        Runnable {
            ...
            mBinding.overlay.foreground = Color.argb(150, 255, 255, 255).toDrawable()
            // Wait for ANIMATION_FAST_MILLIS
            mBinding.overlay.postDelayed({
                if (isCurrentlyRecording()) {
                    // Remove white flash animation
                    mBinding.overlay.foreground = null
        ...
        mBinding.viewFinder.holder.addCallback(object : SurfaceHolder.Callback {
            ...
            override fun surfaceCreated(holder: SurfaceHolder) {
                ...
                val previewSize = getPreviewOutputSize(
                    mBinding.viewFinder.display, characteristics, SurfaceHolder::class.java
                )
                Log.d(
                    TAG,
                    "View finder size: ${mBinding.viewFinder.width} x ${mBinding.viewFinder.height}"
                )
    ...
    private fun initializeCamera() = lifecycleScope.launch(Dispatchers.Main) {
        ...
        mBinding.captureButton.setOnTouchListener { view, event ->
            when (event.action) {
                ...
                MotionEvent.ACTION_DOWN -> lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    if (!recordingStarted) {
                        ...
                        mBinding.overlay.post(animationTask)
                    }
                }
                ...
                MotionEvent.ACTION_UP -> lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    mBinding.captureButton.setOnTouchListener(null)
