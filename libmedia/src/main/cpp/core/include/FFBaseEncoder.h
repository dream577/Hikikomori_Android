...
protected:
     ...
     * 用于存放原始数据的队列
     */
    shared_ptr<LinkedBlockingQueue<AVMediaFrame>> m_EncoderQueue;
...
public:
    ...
    FFBaseEncoder(AVCodecID encodec_id) {
        this->m_EncoderQueue = make_shared<LinkedBlockingQueue<AVMediaFrame>>(INT_MAX);
     ...
     * @param frame 原始音视频数据
     */
    int InputFrame(shared_ptr<AVMediaFrame> frame) {
