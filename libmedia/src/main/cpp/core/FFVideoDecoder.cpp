...
FFVideoDecoder::FFVideoDecoder(AVDecoderCallback *callback, double timebase)
...
std::shared_ptr<AVMediaFrame> FFVideoDecoder::_OnFrameAvailable(AVFrame *avFrame) {
    ...
    switch (avFrame->format) {
        case AV_PIX_FMT_YUV420P: {
            ...
            if (!mOutputFramePool) {
                auto fun = [&](int i) -> shared_ptr<AVMediaFrame> {
                    auto f = make_shared<AVMediaFrame>();
                ...
                mOutputFramePool = make_shared<LinkedBlockingQueue<AVMediaFrame>>(VIDEO_FRAME_POOL_SIZE, fun);
            }

            frame = mOutputFramePool->poll();
            if (!frame) break;
            frame->pool = mOutputFramePool;
