...
GLRenderWindow::GLRenderWindow(AVRenderCallback *callback) {
...
void GLRenderWindow::OnDrawFrame(std::shared_ptr<AVMediaFrame> frame) {
    if (glRender) {
...
void GLRenderWindow::_OnDrawFrame() {
    shared_ptr<AVMediaFrame> frame =  m_Callback->GetOneFrame(AVMEDIA_TYPE_VIDEO);
    if (frame) {
