...
int VioletMediaPlayer::Init(JNIEnv *jniEnv, jobject obj, char *url, int decodeType,
    ...
    m_VideoRender = make_shared<GLRenderWindow>(this);
    m_VideoFrameQueue = make_shared<LinkedBlockingQueue<AVMediaFrame>>(MAX_VIDEO_FRAME_SIZE);

    m_AudioRender = make_shared<OpenSLAudioRender>(this);
    m_AudioRender->Init();
    m_AudioFrameQueue = make_shared<LinkedBlockingQueue<AVMediaFrame>>(MAX_AUDIO_FRAME_SIZE);
...
void VioletMediaPlayer::OnSurfaceCreated(JNIEnv *jniEnv, jobject surface) {
    if (m_VideoRender) {
        m_VideoRender->OnSurfaceCreated(jniEnv, surface);
    }
}
...
void VioletMediaPlayer::OnSurfaceDestroyed() {
    if (m_VideoRender) {
        m_VideoRender->OnSurfaceDestroyed();
    }
}
...
void VioletMediaPlayer::UpdateMVPMatrix(float translateX, float translateY, float scaleX,
    ...
    if (m_VideoRender) {
        m_VideoRender->UpdateMVPMatrix(translateX, translateY, scaleX,
...
void VioletMediaPlayer::Play() {
    ...
    m_InputEngine->StartLoop();
    m_AudioRender->StartLoop();
    m_VideoRender->StartLoop();
}
...
void VioletMediaPlayer::Stop() {
    LOGCATE("VioletMediaPlayer::StopDecoder")
...
shared_ptr<AVMediaFrame> VioletMediaPlayer::GetOneFrame(int type) {
//    LOGCATE("VioletMediaPlayer::GetOneFrame type=%d", type)
    shared_ptr<AVMediaFrame> frame;
...
void VioletMediaPlayer::FrameRendFinish(shared_ptr<AVMediaFrame> frame) {
    if (frame) {
        // TODO 此处用来扩展录制模块，暂时搁置
        if (frame->type == AVMEDIA_TYPE_VIDEO) {
            AVMediaFrame::recycle(frame);
        } else {
...
void VioletMediaPlayer::OnFrameReady(std::shared_ptr<AVMediaFrame> frame) {
